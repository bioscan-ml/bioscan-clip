...
def make_txt_features(model, all_species, species_to_other, openai_templates):
    tokenizer = open_clip.get_tokenizer("hf-hub:imageomics/bioclip")

    concat_str = [species_to_other[species]['order'] + " " + species_to_other[species]['family'] + " " + species_to_other[species]['genus'] + " " + species_to_other[species]['species']
                   for species in all_species]
    with torch.no_grad():
        txt_features = []
        for text in tqdm(concat_str):
            text = " ".join(word for word in text.split("_") if word)
            texts = [template(text) for template in openai_templates]  # format with class
...
def main(args: DictConfig) -> None:
    ...
    txt_features_of_all_species = make_txt_features(model, all_species, species_to_other, openai_templates)
